[simple_galaxy.html](https://github.com/user-attachments/files/23565506/simple_galaxy.html)
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        body {
            ...
            background: linear-gradient(135deg, #1e3a8a 0%, #0c4a6e 100%); /* Fondo azul oscuro galáctico */
            color: #e0f2f1;
            min-height: 100vh;
            display: flex;
    ...
    <div class="card-container">
        ...
        <header class="text-center mb-8">
            <h1 class="text-4xl sm:text-5xl font-extrabold text-[#93c5fd] tracking-wide">
                🚀 Simple Present Galaxy 🌟
            </h1>
            <p class="text-xl mt-2 text-gray-300">Aprende y Domina el Presente Simple en Inglés</p>
        </header>

        <!-- Navegación y Contenido -->
        <main>
            <!-- Contenedor para la navegación (botones) -->
            <nav class="flex flex-wrap justify-center gap-4 mb-8">
                <button onclick="changeSection('explicacion')" class="btn-primary">📖 Explicación</button>
                <button onclick="changeSection('reglas_s')" class="btn-primary">💡 Regla de la 'S'</button>
                <button onclick="changeSection('estructuras')" class="btn-primary">📐 Estructuras</button>
                <button onclick="changeSection('juego1')" class="btn-game">🕹️ Misión 1: S/ES</button>
                <button onclick="changeSection('juego2')" class="btn-game">🎮 Misión 2: Negativas</button>
            </nav>
        ...
        <!-- Área de mensajes de juego o feedback -->
        <div id="feedback-area" class="mt-6 text-center text-2xl font-bold p-4 rounded-xl hidden"></div>

        <!-- Pie de página (Footer) -->
        <footer class="mt-8 text-center text-gray-400 text-sm">
            Diseñado para la práctica interactiva del Presente Simple. ¡Sigue explorando!
        </footer>
    </div>

    <!-- JavaScript para la interactividad -->
    <script>
        // Objeto que almacena el contenido de cada sección
        const sections = {
            explicacion: `
                <h2 class="text-3xl font-bold mb-4 text-blue-400">¿Qué es el Simple Present?</h2>
                <p class="mb-4">Lo usamos para dos cosas principales:</p>
                <ul class="list-disc list-inside space-y-2 ml-4">
                    <li>✅  **Hábitos y Rutinas:** Cosas que haces regularmente (e.g., *I drink coffee every morning.*).</li>
                    <li>✅  **Hechos Generales:** Verdades que siempre son ciertas (e.g., *The sun rises in the East.*).</li>
                </ul>
            ...
            `,
            reglas_s: `
                <h2 class="text-3xl font-bold mb-4 text-green-400">La Regla de la 'S' (¡La más importante!)</h2>
                ...
                <ul class="list-disc list-inside space-y-2 ml-4">
                    <li>Verbos que terminan en **-ss, -sh, -ch, -x, -o, -z** (e.g., *watch, go, kiss*): Se añade **-ES** (He watch<span class="text-red-400">es</span>, She go<span class="text-red-400">es</span>).</li>
            ...
            `,
            estructuras: `
                <h2 class="text-3xl font-bold mb-4 text-red-400">Estructuras Básicas</h2>
                <div class="space-y-4">
                    <!-- Afirmativa -->
                    ...
                    <!-- Interrogativa -->
                    <div class="p-4 bg-gray-700 rounded-lg border-l-4 border-red-500">
                        <p class="font-bold text-xl mb-2 text-red-300">3. Interrogativa (?) - Preguntas de Sí/No</p>
            ...
            juego1: `
                <h2 class="text-3xl font-bold mb-6 text-yellow-400">Misión 1: La Regla de la 'S' (Tercera Persona)</h2>
                <p class="text-lg mb-4">Elige la forma verbal correcta para completar la oración.</p>
                ...
                <div id="game1-options" class="grid grid-cols-2 gap-4">
                    <!-- Opciones de botones se cargarán aquí -->
                </div>
                <div id="game1-result" class="mt-4 text-center font-bold text-xl min-h-[3rem]"></div>
            `,
            juego2: `
                <h2 class="text-3xl font-bold mb-6 text-purple-400">Misión 2: Negativas (Do not / Does not)</h2>
        ...
        /**
         * Función principal para cambiar el contenido de la sección
        ...
        function changeSection(sectionId) {
            ...
            // Si es una sección de explicación, cargar el HTML estático
        ...
        // --- Lógica del Juego 1 (Regla de la 'S') ---
        ...
        function loadGame1Question() {
            if (currentQIndex1 >= game1Questions.length) {
                document.getElementById('game1-question').innerHTML = '¡Misión Completada! 🏆';
                document.getElementById('game1-options').innerHTML = '<p class="col-span-2 text-center text-green-400 font-bold">¡Lo has logrado! Vuelve a la explicación o intenta la Misión 2.</p>';
            ...
            // Asegurarse de que las opciones no sean idénticas
            let options = [data.correct];
            if (incorrectOption !== data.correct) {
                 options.push(incorrectOption);
            } else {
                // Si son iguales (raro, pero posible en casos edge), añade una base limpia
        ...
        function checkAnswer1(selected, correct) {
            ...
            if (selected === correct) {
                ...
                resultDiv.textContent = '¡Correcto! Sigue así. ✅';
                currentQIndex1++;
            } else {
                resultDiv.className = 'mt-4 text-center font-bold text-xl p-3 rounded-lg incorrect';
                resultDiv.textContent = `Incorrecto. La respuesta correcta era: ${correct}. ❌`;
            }
        ...
        // --- Lógica del Juego 2 (Negativas Do/Does) ---
        ...
        function loadGame2Question() {
            if (currentQIndex2 >= game2Questions.length) {
                document.getElementById('game2-question').innerHTML = '¡Misión Completada! 🏆';
                document.getElementById('game2-options').innerHTML = '<p class="col-span-2 text-center text-purple-400 font-bold">¡Misión de Negativas completada! Vuelve a la explicación o descansa.</p>';
            ...
            if (data.correct.includes('does')) {
                ...
                // Trampa 2: Usar la negación incorrecta del otro pronombre
        ...
        function checkAnswer2(selected, correct) {
            ...
            if (selected === correct) {
                resultDiv.className = 'mt-4 text-center font-bold text-xl p-3 rounded-lg correct';
                resultDiv.textContent = '¡Excelente! Negación Perfecta. ✅';
                currentQIndex2++;
            } else {
                resultDiv.className = 'mt-4 text-center font-bold text-xl p-3 rounded-lg incorrect';
                resultDiv.textContent = `Error. La forma correcta es: ${correct}. Recuerda que DOES NOT se usa con He/She/It. ❌`;
            }
        ...
        // Cargar la sección de explicación al inicio
        window.onload = () => {
